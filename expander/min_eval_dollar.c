...
/*   Updated: 2023/06/22 11:32:22 by lwidmer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

static char	ft_dollar_split_extra(t_lexer **token, t_expander **word,
										t_expander **extra, t_exphelp help)
{
	...
	if (help.space == 3)
		help.space = min_to_aste(word, min_asterisk_splitvalue(
					help.splitvalue[min_len_split(help.splitvalue) - 1]), NULL);
	if (help.space == 4)
...
static char	ft_dollar_split(t_lexer **token, t_expander **word,
								t_expander **extra, t_exphelp help)
{
	if (extra)
...
char	min_eval_dollar(t_lexer **token, t_expander **word,
							t_expander **extra, t_exphelp help)
{
	if (!min_check_word((*token)->value, &help.dollar_value))
	{
		...
		return (min_add_value_to_expander(token, word,
				&help.dollar_value, help.space));
	}
